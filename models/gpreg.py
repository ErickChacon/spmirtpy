...
def gpreg(y, dist, params, niter, Sigma_proposal):
    ...
    params_log_first = tf.Variable(params, dtype = tf.float32)
    ...
    # Computation
    # params_log_posterior = posterior(params_log_first, tf_dis, tf_y, prior_c_sigma2)
    ...
    plop = sess_init.run(posterior(params_log, tf_dis, tf_y, prior_c_sigma2),
            feed_dict)
    params_log_posterior = tf.Variable(plop, dtype = tf.float32)
